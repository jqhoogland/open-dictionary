...
const getIPA = ({ broad, narrow }: IPAOpts) =>
...
const EntryPage = (props: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  const pronunciationQuery = trpc.useQuery([
    "pronunciation.listByWord",
    { language, word },
  ]);
  const definitionQuery = trpc.useQuery([
    "definition.listByWord",
    { language, word },
  ]);
    ...
    <BasicLayout title={word ? `${word} | OpenDictionary` : "OpenDictionary"}>
      ...
      <main className="px-8 mx-auto max-w-screen-md py-16 min-h-[80vh]">
        <h1 className="text-6xl font-bold">
          {word ?? (
            <div className="h-10 w-[200px] animate-pulse bg-base-300 rounded-xl"></div>
          )}
        </h1>
        ...
        {doesNotExist ? (
          <section className="py-4">
            ...
            <p className="text-stone-500">
              {" "}
              Would you like to create an entry?{" "}
              <a
                className="btn-link text-blue-500"
                href="mailto:jesse@jessehoogland.com"
              >
                Reach out.
              </a>
            </p>
          ...
          <>
            ...
            <section className="py-4">
              ...
              <div className="bg-base-200 rounded-xl p-4 gap-2 w-full max-w-[65ch]">
                {(pronunciationQuery.data ?? []).map(
                  ({ broad, narrow, description }, i) =>
                    (broad || narrow) && (
                      <span className="badge badge-outline mx-2" key={i}>
                        {getIPA({ broad, narrow })}
                      </span>
                    )
                )}
...
export async function getStaticProps({
  params,
  ...ctx
}: GetStaticPropsContext<WordAndLanguage>) {
