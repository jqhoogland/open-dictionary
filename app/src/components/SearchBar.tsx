...
const SearchBar: React.FC<SelectLanguageProps> = ({
  onSelectLanguage,
  className,
  defaultValue,
}) => {
  ...
  const handleKeyDown = React.useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.code === "Enter") {
        e.preventDefault();
        router.push(
          `/${activeLanguage}/${(e.target as HTMLInputElement).value}`
        );
      }
    },
    []
  ...
  return (
    <fieldset className={"relative " + (className ?? "")}>
      ...
      />
      <SelectLanguage
        className="absolute !min-h-0 h-auto py-0.5 pl-4 pr-8 top-2 bottom-2 right-2"
