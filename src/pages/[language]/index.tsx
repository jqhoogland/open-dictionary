    ...
    return (
        <table className="mt-8 w-full">
            ...
            <tfoot >
                <tr>
                    <td colSpan={2}>
                        <button onClick={onLoadMore} className="btn btn-outline my-8">Load more</button>
                    </td>
                </tr>
            </tfoot>
        </table>
...
const LanguageOverview = (props: InferGetStaticPropsType<typeof getStaticProps>) => {
    const router = useRouter();
    const { language } = props
    const { data, fetchNextPage, ...rest } = trpc.useInfiniteQuery(['entry.paginate', { language, limit: "10" }], {
        getNextPageParam: (lastPage) => lastPage?.nextCursor.toString(),
    });
    const entries = React.useMemo(() => (data?.pages ?? []).flatMap((({ items }) => items)), [data])
...
export async function getStaticProps({ params, ...ctx }: GetStaticPropsContext<{ language: Language }>) {
